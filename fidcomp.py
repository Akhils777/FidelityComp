...
__all__ = [
    ...
    'superoperator_fidelity', 'kraus_fidelity'
]
...
def state_fidelity(target: Qobj, achieved: Qobj) -> float:
    ...
    Example:
        >>> target = Qobj([[1, 0], [0, 0]])
        >>> achieved = Qobj([[0.9, 0.1], [0.1, 0.9]])
        >>> state_fidelity(target, achieved)
        0.81
    """
    if target.isket:
        target = ket2dm(target)
    if achieved.isket:
        achieved = ket2dm(achieved)
    return fidelity(target, achieved)
...
def get_fidelity_func(kind: str = 'state') -> Union[Callable, None]:
    ...
    Args:
        kind (str): The fidelity type ('state', 'unitary', 'average', 'super', 'kraus', 'custom').

    Returns:
        Callable: The corresponding fidelity function or None if unsupported.

    Example:
        >>> get_fidelity_func('state')
        <function state_fidelity at 0x7f...>
    """
    return {
...
class FidelityTracker:
    def __init__(self, save_path: Union[str, None] = None):
        """
        Initializes the FidelityTracker class.

        Args:
            save_path (Union[str, None]): Path to save the fidelity history. If None, no saving occurs.

        Example:
            >>> tracker = FidelityTracker(save_path="fidelity.json")
        """
        self.history = []
...
def validate_qobj_pair(A: Qobj, B: Qobj, kind: str):
    ...
    Args:
        ...
        kind (str): The type of fidelity ('state', 'unitary', 'average', 'super', 'kraus').
